...
public class SoarParser implements SoarParserConstants {
  ...
  void handleDocumentationQuote(SoarProductionAst sp) throws ParseException {
...
// Ok
  final public void conditionSide(SoarProductionAst sp) throws ParseException {
        Condition cond;
    label_1:
    while (true) {
      cond = condition();
...
// Ok
  ...
  public SoarParser(java.io.InputStream stream, String encoding) {
    try { jj_input_stream = new SoarCharStream(new InputStreamReader(stream, encoding), 0); } catch(java.io.UnsupportedEncodingException e) { throw new RuntimeException(e); }
    token_source = new CustomSoarParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
  ...
  public SoarParser(java.io.Reader stream) {
    jj_input_stream = new SoarCharStream(stream, 0);
    token_source = new CustomSoarParserTokenManager(jj_input_stream);
    token = new Token();
    jj_ntk = -1;
    jj_gen = 0;
