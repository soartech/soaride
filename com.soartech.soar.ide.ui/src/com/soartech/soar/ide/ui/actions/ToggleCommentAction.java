 ...
 * see org.eclipse.jdt.internal.ui.javaeditor.ToggleCommentAction
 */
public class ToggleCommentAction
   ...
   /**
    * Constructor for a <code>ToggleCommentAction</code> object.
    * 
    * @param bundle
    *           resource bundle
    * @param prefix
    ...
    */
   public ToggleCommentAction( ResourceBundle bundle,
                               String prefix,
                               ITextEditor editor ) {
   ...
   public void update() {
      ...
      if( target == null && editor != null ) {
         target = (ITextOperationTarget)
                  editor.getAdapter( ITextOperationTarget.class );
      }
      
      boolean enabled = (target != null &&
                         target.canDoOperation( ITextOperationTarget.PREFIX ) &&
                         target.canDoOperation( ITextOperationTarget.STRIP_PREFIX ) );
   ...
   public void run() {
      ...
      if( target == null || prefixesMap == null ) { return; }
      ...
      if( ! target.canDoOperation( operationCode ) ) {
         if( shell != null ) {
            MessageDialog.openError( shell,
                                     SoarEditorMessages.ToggleComment_error_title,
                                     SoarEditorMessages.ToggleComment_error_message );
         }
         return;
      }
      ...
      try {
         ...
         SoarEditorUIPlugin plugin = SoarEditorUIPlugin.getDefault();
   ...
   private boolean isSelectionCommented( ISelection selection ) {
      ...
      if( prefixes != null && prefixes.length > 0 ) {
         return isBlockCommented( textSelection.getStartLine(),
   ...
   private boolean isBlockCommented( int startLine, int endLine, String[] prefixes, IDocument document ) {
      ...
      try {
         ...
         SoarEditorUIPlugin.log( exception );
      }
    ...
    *         selection
    */
   private ITypedRegion getTextBlockFromSelection( ITextSelection selection,
                                                   IDocument document ) {

      try {
         IRegion line =
            document.getLineInformationOfOffset( selection.getOffset() );
         int length =
            selection.getLength() == 0 ? line.getLength()
               : selection.getLength() +
                 ( selection.getOffset() - line.getOffset() );
         return new TypedRegion( line.getOffset(), length,
                                 document.getContentType( selection.getOffset() ) );
      }
      catch( org.eclipse.jface.text.BadLocationException exception ) {
         SoarEditorUIPlugin.log( exception );
      }
